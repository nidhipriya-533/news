...
useEffect(() => {
  document.title = `${capitalizeFirstLetter(props.category)} - NewsMunk`;
  updateNews();
...
const handleNextClick = async ()=>{
 ...
 const fetchMoreData = async () => {
  const url = `http://newsapi.org/v2/top-headlines?country=${props.country}&category=${props.category}&apiKey=e7e74c3b93324a8a92d762a461a26e91&page=${page+1}&pageSize=${props.pageSize}`;
    ...
    return (
      <>
        <h1 className="text-center" style={{margin: '35px 0px', marginTop: '90px'}}>NewsMunk - Top  {capitalizeFirstLetter(props.category)} Headlines </h1>
         {loading && <Spinner/>} 
        {/* Infinite scroll scroll krne ke liye use krte h ar ye ushka function h */}
        <InfiniteScroll
